    ...
    <div class="product-cart-list">
        ...
        <div class="product cart-product">
            ...
            <div class="product-control-buttons">
                <a href="#" data-id="<?= $product["id"] ?>" class="do-link-plus"><span class="lnr lnr-plus-circle"></span></a>
                <input type="text" id="input-count-<?= $product["id"] ?>" value="<?=$product["quality"]?>" class="input-count"/>
                <a href="#" data-id="<?= $product["id"] ?>" class="do-link-minus"><span class="lnr lnr-circle-minus"></span></a>
            </div>

            <a href="#" data-id="<?= $product["id"] ?>" class="do-link-remove-fro-cart"><span class="lnr lnr-cross-circle"></span></a>
        </div>
        <!-- /Отдельный класс товара -->
        <?php endforeach; ?>
...
<script>
    $(document).ready(function () {
        $(".do-link-remove-fro-cart").on("click", function (event) {
            ...
            $.post("http://localhost/cart/remove/" + id, {}, function (reply) {
                location.reload();
                console.log(reply);
                $("#product-" + id).remove();
